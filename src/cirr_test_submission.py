...
def generate_cirr_test_dicts(relative_test_dataset: CIRRDataset, clip_model: CLIP, index_features: torch.tensor,
    ...
    # Normalize the index features to compute cosine similarity
    index_features = F.normalize(index_features, dim=-1).float()

    # Compute the distances and sort the results
    distances = 1 - predicted_features @ index_features.T # Compute (1-cosine similarity) between predicted features and index features
    sorted_indices = torch.argsort(distances, dim=-1).cpu() # Sort the distances in ascending order (tăng dần)
    sorted_index_names = np.array(index_names)[sorted_indices] # Get the index names of the sorted distances
    ...
    group_members = np.array(group_members) # danh sách các ảnh trong cùng group (candidates hợp lệ)
    group_mask = (sorted_index_names[..., None] == group_members[:, None, :]).sum(-1).astype(bool) # Ftìm các ảnh trong sorted_index_names thuộc group_members
    sorted_group_names = sorted_index_names[group_mask].reshape(sorted_index_names.shape[0], -1) # chỉ giữ các ảnh trong group, vẫn theo thứ tự distance

    # Generate prediction dicts
    pairid_to_predictions = {str(int(pair_id)): prediction[:50].tolist() for (pair_id, prediction) in
                             zip(pairs_id, sorted_index_names)} # global retrieval results
    pairid_to_group_predictions = {str(int(pair_id)): prediction[:3].tolist() for (pair_id, prediction) in
                                   zip(pairs_id, sorted_group_names)} # subset retrieval results
...
def generate_cirr_test_predictions(clip_model: CLIP, relative_test_dataset: CIRRDataset, combining_function: callable,
    ...
    relative_test_loader = DataLoader(dataset=relative_test_dataset, batch_size=32,
                                      num_workers=multiprocessing.cpu_count(), pin_memory=False) # Load data (pairs_id, reference_names, captions, group_members) in batches

    # Get a mapping from index names to index features (mapping từ tên ảnh sang features tương ứng)
    name_to_feat = dict(zip(index_names, index_features)) 

    # Initialize pairs_id, predicted_features, group_members and reference_names
    pairs_id = [] # lưu pair_id của từng query
    predicted_features = torch.empty((0, clip_model.visual.output_dim)).to(device, non_blocking=True) # lưu predicted features của từng query, tensor rỗng, append từng batch
    group_members = [] # lưu group_members của từng query
    reference_names = [] # lưu reference image của từng query (để xóa ảnh reference khỏi kết quả)

    # Load data (pairs_id, reference_names, captions, group_members) in batches
    for batch_pairs_id, batch_reference_names, captions, batch_group_members in tqdm(
        ...
        with torch.no_grad():
            text_features = clip_model.encode_text(text_inputs) # encode captions với clip --> shape (batch_size, 768)
            # lấy features của reference image từ index features
            if text_features.shape[0] == 1:
                # batch_size = 1
                reference_image_features = itemgetter(*batch_reference_names)(name_to_feat).unqueeze(0)
            else:
                # batch_size > 1 (xử lý riêng vì torch.stack không hoạt động với single tensor)
        ...
        group_members.extend(batch_group_members) # lưu group_members của từng query
        reference_names.extend(batch_reference_names) # lưu reference image của từng query
        pairs_id.extend(batch_pairs_id) # lưu pair_id của từng query
...
def main():
    ...
    # Load the default CLIP model (RN50x16)
    clip_model, clip_preprocess = clip.load(args.clip_model_name, device=device, jit=False)
    input_dim = clip_model.visual.input_resolution # kích thước ảnh đầu vào của CLIP
    feature_dim = clip_model.visual.output_dim # kích thước features của CLIP
    ...
    if args.transform == 'targetpad':
        ...
        preprocess = targetpad_transform(args.target_ratio, input_dim) # resize với target ratio (tỉ lệ khung hình), giữ nguyên aspect ratio
    elif args.transform == 'squarepad':
        print('Square pad preprocess pipeline is used')
        preprocess = squarepad_transform(input_dim) # resize về square
    else:
        print('CLIP default preprocess pipeline is used')
        preprocess = clip_preprocess # sử dụng pipeline mặc định của CLIP

    # Setup the combining function
    if args.combining_function.lower() == 'sum':
        ...
        combining_function = element_wise_sum # dùng element-wise sum
        ...
        combiner.eval()
        combining_function = combiner.combine_features # dùng combiner moe đã huấn luyện
    else:
        raise ValueError("combiner_path should be in ['sum', 'combiner']")

    generate_cirr_test_submissions(combining_function, args.submission_name, clip_model, preprocess) # generate test submission files to be submitted to evaluation SERVER
