...
# Trích xuất đặc trưng của dataset dùng CLIP đã fine-tuned
def extract_index_features(dataset: Union[CIRRDataset, FashionIQDataset], clip_model: CLIP) -> \
    ...
    return index_features, index_names # return tensor features và image names
...
# Tạo caption ngẫu nhiên từ 2 captions của FashionIQ để tăng đa dạng dữ liệu
def generate_randomized_fiq_caption(flattened_captions: List[str]) -> List[str]:
    ...
    for i in range(0, len(flattened_captions), 2):
        ...
        if random_num < 0.25:
            captions.append(
                f"{flattened_captions[i].strip('.?, ').capitalize()} and {flattened_captions[i + 1].strip('.?, ')}") # (a) cap1 and cap2
        elif 0.25 < random_num < 0.5:
            captions.append(
                f"{flattened_captions[i + 1].strip('.?, ').capitalize()} and {flattened_captions[i].strip('.?, ')}") # (b) cap2 and cap1
        elif 0.5 < random_num < 0.75:
            captions.append(f"{flattened_captions[i].strip('.?, ').capitalize()}") # (c) cap1
        else:
            captions.append(f"{flattened_captions[i + 1].strip('.?, ').capitalize()}") # (d) cap2
    return captions

# Loại bỏ none images trong batch khi dùng DataLoader
...
# Cập nhật kết quả training trong từng epoch
...
# Cập nhật mô tả của tqdm trong training
