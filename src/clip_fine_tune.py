...
# ----------- Clip fine-tuning with FashionIQ dataset -----------
def clip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    # Tạo thư mục lưu model theo timestamp, lưu hyperparameters vào JSON
    ...
    clip_model, clip_preprocess = clip.load(clip_model_name, device=device, jit=False) # load CLIP model
    ...
    # kích thước ảnh đầu vào của CLIP

    if transform == "clip": #default pipeline
        preprocess = clip_preprocess
        print('CLIP default preprocess pipeline is used')
    elif transform == "squarepad": #resize về square
        preprocess = squarepad_transform(input_dim)
        print('Square pad preprocess pipeline is used')
    elif transform == "targetpad": #resize với target ratio (tỉ lệ khung hình), giữ nguyên aspect ratio
    ...
    combining_function = element_wise_sum # dùng element-wise sum để combine features của reference image và captions

    # Define the optimizer, the loss and the grad scaler
    optimizer = optim.AdamW( # sử dụng AdamW optimizer
        [{'params': filter(lambda p: p.requires_grad, clip_model.parameters()), 'lr': learning_rate,
          'betas': (0.9, 0.999), 'eps': 1e-7}])
    crossentropy_criterion = nn.CrossEntropyLoss() # sử dụng cross entropy loss
    scaler = torch.cuda.amp.GradScaler()

    # When save_best == True initialize the best result to zero
    if save_best: # nếu save_best == True, khởi tạo best_avg_recall = 0
        best_avg_recall = 0

    # Define dataframes for CSV logging
    training_log_frame = pd.DataFrame() # DataFrame để lưu log training
    validation_log_frame = pd.DataFrame() # DataFrame để lưu log validation
    ...
    for epoch in range(num_epochs):
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):
                ...
                # encode reference image, relative caption và target image với CLIP
                reference_images = reference_images.to(device, non_blocking=True)
                # encode target image với CLIP
                ...
                with torch.cuda.amp.autocast():
                    ...
                    logits = 100 * predicted_features @ target_features.T # compute logits

                    ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                    loss = crossentropy_criterion(logits, ground_truth) # compute loss  

                # Backpropagate and update the weights. Mixed precision với GradScaler
        ...
        if epoch % validation_frequency == 0:
            with experiment.validate():
                ...
                for relative_val_dataset, classic_val_dataset, idx in zip(relative_val_datasets, classic_val_datasets,
                    ...
                    recall_at10, recall_at50 = compute_fiq_val_metrics(relative_val_dataset, clip_model, # tính recall_at10 và recall_at50 cho từng category
                                                                       index_features, index_names,
                                                                       combining_function)
                    recalls_at10.append(recall_at10) # lưu recall_at10 cho từng category
                    recalls_at50.append(recall_at50) # lưu recall_at50 cho từng category

                results_dict = {} # tạo dictionary để lưu kết quả validation
                for i in range(len(recalls_at10)):
                    results_dict[f'{idx_to_dress_mapping[i]}_recall_at10'] = recalls_at10[i] # lưu recall_at10 cho từng category
                    results_dict[f'{idx_to_dress_mapping[i]}_recall_at50'] = recalls_at50[i] # lưu recall_at50 cho từng category
                results_dict.update({
                    f'average_recall_at10': mean(recalls_at10), # tính average recall_at10 cho tất cả categories
                    f'average_recall_at50': mean(recalls_at50), # tính average recall_at50 cho tất cả categories
                    f'average_recall': (mean(recalls_at50) + mean(recalls_at10)) / 2 # tính average recall cho tất cả categories
                })

                print(json.dumps(results_dict, indent=4)) # in ra results_dict dưới dạng JSON
            ...
            if save_training: # nếu save_training == True, lưu model nếu đạt best performance
                if save_best and results_dict['average_recall'] > best_avg_recall: # nếu save_best == True và average_recall > best_avg_recall, lưu model
                    best_avg_recall = results_dict['average_recall']
                    save_model('tuned_clip_best', epoch, clip_model, training_path) # lưu model với tên tuned_clip_best
                elif not save_best:
                    save_model(f'tuned_clip_{epoch}', epoch, clip_model, training_path) # lưu model với tên tuned_clip_{epoch}


# Tương tự với clip_finetune_fiq, nhưng cho CIRR dataset
