...
from combiner_moe import Combiner # Combiner thiết kế của đồ án
...
# ----------- Combiner training with FashionIQ dataset -----------
def combiner_training_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    if transform == "clip":
        preprocess = clip_preprocess # default pipeline
        ...
        preprocess = targetpad_transform(target_ratio, input_dim) # resize với target ratio (tỉ lệ khung hình), giữ nguyên aspect ratio
        print(f'Target pad with {target_ratio = } preprocess pipeline is used')
    else:
        raise ValueError("Preprocess transform should be in ['clip', 'squarepad', 'targetpad']")

    if kwargs.get("clip_model_path"): # nếu có path đến model CLIP đã huấn luyện, load model đó
        print('Trying to load the CLIP model')
        clip_model_path = kwargs["clip_model_path"]
        saved_state_dict = torch.load(clip_model_path, map_location=device) # load state dict từ file
        clip_model.load_state_dict(saved_state_dict["CLIP"]) # load state dict vào model
        print('CLIP model loaded successfully') 

    clip_model = clip_model.float() # convert model to float32
...
# ----------- Combiner training with CIRR dataset (tương tự như với FashionIQ dataset) -----------
def combiner_training_cirr(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str,
    ...
    relative_train_loader = DataLoader(dataset=relative_train_dataset, batch_size=batch_size, num_workers=0,
