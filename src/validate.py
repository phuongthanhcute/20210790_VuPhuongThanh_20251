...
# Đánh giá trên validation set (có ground truth), tính metrics.
...
def generate_fiq_val_predictions(clip_model: CLIP, relative_val_dataset: FashionIQDataset,
    ...
    name_to_feat = dict(zip(index_names, index_features)) # map index names to index features
...
# Truy xuất trên tập validation FashionIQ cho một category.
def fashioniq_val_retrieval(dress_type: str, combining_function: callable, clip_model: CLIP, preprocess: callable):
    ...
    classic_val_dataset = FashionIQDataset('val', [dress_type], 'classic', preprocess) # load dataset classic
    index_features, index_names = extract_index_features(classic_val_dataset, clip_model) # trích xuất features index
    relative_val_dataset = FashionIQDataset('val', [dress_type], 'relative', preprocess) # load dataset relative

    return compute_fiq_val_metrics(relative_val_dataset, clip_model, index_features, index_names,
                                   combining_function) # tính metrics
...
def compute_cirr_val_metrics(relative_val_dataset: CIRRDataset, clip_model: CLIP, index_features: torch.tensor,
    ...
    :return: the computed validation metrics
    """
    # Tạo predicted features cho CIRR validation.
    ...
    # Tính toán các nhãn ground truth dựa trên các group members (subset predictions)
...
def generate_cirr_val_predictions(clip_model: CLIP, relative_val_dataset: CIRRDataset,
    ...
    for batch_reference_names, batch_target_names, captions, batch_group_members in tqdm(
        ...
        text_inputs = clip.tokenize(captions).to(device, non_blocking=True) # tokenize captions với CLIP
        ...
        with torch.no_grad():
            ...
            # a single tensor
            if text_features.shape[0] == 1:
                reference_image_features = itemgetter(*batch_reference_names)(name_to_feat).unsqueeze(0) # lấy features của reference image từ index features
            else:
    ...
    return predicted_features, reference_names, target_names, group_members # return predicted features, reference names, target names and group members
...
def cirr_val_retrieval(combining_function: callable, clip_model: CLIP, preprocess: callable):
    ...
    # Define the validation datasets and extract the index features
    classic_val_dataset = CIRRDataset('val', 'classic', preprocess) # load dataset classic
    index_features, index_names = extract_index_features(classic_val_dataset, clip_model) # trích xuất features index
    relative_val_dataset = CIRRDataset('val', 'relative', preprocess) # load dataset relative
...
def main():
    ...
    # Load CLIP model đã fine-tuned
    if args.clip_model_path:
        print('Trying to load the CLIP model')
        saved_state_dict = torch.load(args.clip_model_path, map_location=device) # load state dict từ file
        clip_model.load_state_dict(saved_state_dict["CLIP"]) # load state dict vào model
        print('CLIP model loaded successfully')     

    # Chọn preprocess pipeline
    ...
    # Chọn combiner moe đã huấn luyện
        ...
        state_dict = torch.load(args.combiner_path, map_location=device) # load state dict từ file  
    ...
    # Chạy retrieval trên tập validation CIRR
    ...
    # Chạy retrieval trên tập validation FashionIQ
    elif args.dataset.lower() == 'fashioniq':
        average_recall10_list = []
        average_recall50_list = []

        # Chạy retrieval trên tập validation FashionIQ cho category 'shirt'
        ...
        # Chạy retrieval trên tập validation FashionIQ cho category 'dress'
        ...
        # Chạy retrieval trên tập validation FashionIQ cho category 'toptee'
