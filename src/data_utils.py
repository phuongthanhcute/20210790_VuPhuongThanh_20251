...
# Convert ảnh sang RGB do CLIP yêu cầu ảnh RGB
def _convert_image_to_rgb(image):
    return image.convert("RGB")

# Padding ảnh --> ảnh vuông với zero padding: lấy cạnh lớn hơn, thêm padding đều 2 bên
class SquarePad:
    """
...
# Padding ảnh với tỉ lệ mong muốn (target ratio). Chỉ pad khi tỉ lệ thực tế > target ratio
class TargetPad:
    """
...
#  Pipeline xử lý ảnh theo kiểu CLIP: squarepad --> resize --> center crop --> convert to RGB --> to tensor --> normalize (mean, std)
...
# Tương tự nhưng dùng TargetPad thay cho squarepad
...
class FashionIQDataset(Dataset):
    ...
    def __init__(self, split: str, dress_types: List[str], mode: str, preprocess: callable):
        ...
        # Load triplets made by (reference_image, target_image, a pair of relative captions)
...
#  Xử lứ tương tự như FashionIQDataset
